...
{   
    if(_has_map ) return;
    // std::cout << "point callback12" << std::endl;
...
{
    ros::init(argc, argv, "rrtstar_demo_node");
    ros::NodeHandle nh("~");
    std::cout << "1" << std::endl;

    _map_sub  = nh.subscribe( "/rrtstar_random_complex/global_map", 1, rcvPointCloudCallBack );
...
void visRRTstarPath(vector<Vector3d> nodes )
{
    ...
    Points.ns              = Line.ns              = "rrtstar_demo_node/RRTstarPath";
